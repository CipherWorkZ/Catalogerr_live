...
# ---------------- Drive Insert ---------------- #
def insert_drive(conn, name, path):
    drive_id = sha1_str(path)

    # Check if drive already exists
    cur = conn.cursor()
    cur.execute("SELECT id FROM drives WHERE id=?", (drive_id,))
    row = cur.fetchone()

    if row:
        # Update only path if needed, preserve metadata
        conn.execute("UPDATE drives SET path=? WHERE id=?", (path, drive_id))
    else:
        # New drive, insert with empty metadata
        conn.execute("""
        INSERT INTO drives (id, path, device, brand, model, serial, total_size)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (drive_id, path, None, None, None, None, None))
